...
from main_Sim import args_config_Sim
    ...
    for idx in range(n_show):
        ...
        for gid, group in enumerate(true_groups):
            ...
            for i,pid in enumerate(group):
                ...
                ax_true.plot(traj[:-1, 0], traj[:-1, 1], linestyle='-', color=color, label=label)
                ax_true.scatter(traj[:-2, 0], traj[:-2, 1], color=color, s=30)
                ax_true.annotate('', xy=traj[-1], xytext=traj[-2],
                                 arrowprops=dict(arrowstyle='->,head_width=0.5,head_length=0.8', linewidth=2.5, color=color))
        ...
        for gid, group in enumerate(pred_groups):
            ...
            for i,pid in enumerate(group):
                traj = coords[pid]
                label = f'Group {gid}' if i == 0 else None
                ax_pred.plot(traj[:-1, 0], traj[:-1, 1], linestyle='-', color=color, label=label)
                ax_pred.scatter(traj[:-2, 0], traj[:-2, 1], color=color, s=30)
                ax_pred.annotate('', xy=traj[-1], xytext=traj[-2],
                                 arrowprops=dict(arrowstyle='->,head_width=0.5,head_length=0.8', linewidth=2.5, color=color))
...
def test_model(model, test_loader, tag="Test"):
    ...
    if vis_data:
        visualize_samples(vis_data, vis_preds, vis_trues, metrics_list,vis_embeddings,
                          f"./visualization/SIM/visual_results_{tag}.svg",tag,
...
if __name__ == "__main__":
    ...
    MODEL_PATH = "./saved_models/best_model_Sim.pth"
    num_samples = 10

    train_loader, val_loader, test_loader = data_generator("./data/Sim/", args=args)
