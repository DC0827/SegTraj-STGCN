...
def generate_subsamples(data, adj):
    N = data.shape[0]

    # 小样本和全是孤立样本不扩充
    if N <= 3 or torch.sum(adj) - torch.sum(torch.diag(adj)) == 0:
        label = [int(adj[i, j].item()) for i in range(N) for j in range(N) if i != j]
        label = torch.tensor(label)
        return [(data, label)]

    groups = extract_groups_from_adj(adj)
    subsamples = []
    for num_groups in range(1, len(groups)+1):
        for group_combo in itertools.combinations(groups, num_groups):
            indices = sorted(set().union(*group_combo))
            if len(indices) <= 3:
                continue
            sub_data = data[indices]
            sub_adj = adj[torch.tensor(indices)[:, None], torch.tensor(indices)]
            label = [int(sub_adj[i, j].item()) for i in range(len(indices)) for j in range(len(indices)) if i != j]
            label = torch.tensor(label)
            subsamples.append((sub_data, label))
    return subsamples
...
for idx, (data, label) in enumerate(zip(all_data, all_labels)):
    ...
    subs = generate_subsamples(data, adj)
