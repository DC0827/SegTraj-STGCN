...
from data_loader_SIM import data_generator
from utils import *
...
class Train():
    def __init__(self, args):
        ...
        self.train, self.valid, self.test = data_generator('./data/Sim/', args=args)
    ...
    def compute_loss(self, prediction, label):
        ...
        # 加权 BCE Loss：这里 prediction 是 sigmoid 输出，因此不能用 logits 版
        # bce_loss = F.binary_cross_entropy(
        #     prediction, 
        #     label, 
        #     weight=(label*2.48 + 0.63*(1 - label))  # 正例加权，负例权重   
        # )
        ...
        bce_loss = F.cross_entropy(
            ...
            weight=tr.tensor([0.63, 2.48],device=prediction.device)
        )
    ...
    def Train_model(self):
        ...
        for i in range(epoch):
            ...
            if F1_val > Best_F1:  
                ...
                # 保存模型
                model_filename = os.path.join(MODEL_SAVE_PATH, f"best_model_Sim.pth")
    ...
    def plot_loss(self):
        ...
        # 保存图片
        save_path = os.path.join("./logs/Sim", f"Sim_heads{args.num_heads}_batch{args.batch_size}_epoch{args.epoch}_lr{args.lr}_{args.time0}_阈值0.9_FC方式.png")
...
def args_config_Sim(args):
    ...
    args.log_file_path = f"./logs/Sim/Sim_heads{args.num_heads}_batch{args.batch_size}_epoch{args.epoch}_lr{args.lr}_{args.time0}_阈值0.9_FC方式.txt"
...
if __name__ == '__main__':
    ...
    args = args_config_Sim(args)

    train = Train(args)
    train.Train_model()


    # CUDA_VISIBLE_DEVICES=5 python main_Sim.py
