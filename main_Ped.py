...
class Train():
    ...
    def compute_loss(self, prediction, label,epsilon=1e-7):
        ...
        bce_loss = F.binary_cross_entropy(
            prediction, 
            label, 
            weight=(label * g_w + ng_w*(1 - label)) ) # 正例加权，负例权重  
    ...
    def plot_loss(self):
        ...
        # 保存图片
        save_path = os.path.join("./logs", f"{DATASET_NAME}/{DATASET_NAME}_注意力头数{args.num_heads}_学习率{args.lr}_批次{args.accum_steps}_{args.time0}_阈值0.9_有加速度.png")
...
def args_config_Ped(args):
    ...
    args.batch_size = 1
    args.accum_steps = 500  #真实的batch_size
...
if __name__ == '__main__':
    ...
    # 可变参数,待处理数据集的名称
    DATASET_NAME = "ETH"  #  NBA ETH   Hotel  students03   Sim  Sim_2  Sim_5
    ...
    args.log_file_path = f"./logs/{DATASET_NAME}/{DATASET_NAME}_注意力头数{args.num_heads}_学习率{args.lr}_批次{args.accum_steps}_{ args.time0}_阈值0.9_有加速度.txt"
    train = Train(args)
    train.Train_model()


    # CUDA_VISIBLE_DEVICES=2 python main_Ped.py
